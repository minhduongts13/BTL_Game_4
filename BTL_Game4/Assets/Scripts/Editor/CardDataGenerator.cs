...
public class CardDataGenerator
{
    ...
    public static void GenerateCardAssets()
    {
        ...
        string assetOutputPath = "Assets/Resources/Cards";
        ...
        foreach (string fullPath in spritePaths)
        {
            ...
            if (fileName.ToLower() == "backcard")
            {
                CardData backCard = ScriptableObject.CreateInstance<CardData>();
                backCard.cardColor = "Back";
                backCard.cardNumber = -99; // hoặc giá trị nào đó bạn định nghĩa riêng cho lưng bài
                backCard.cardSprite = sprite;

                string backAssetName = "BackCard.asset";  // ← sửa tên biến ở đây
                string savebackPath = Path.Combine(assetOutputPath, backAssetName);

                AssetDatabase.CreateAsset(backCard, savebackPath);
                Debug.Log($"Đã tạo lá bài úp: {savebackPath}");
